    ...
    for x in range(0,count):
        ...
        if (type == 'A' or type == 'W' ):
            ...
            while(time.perf_counter() - time_start < interval):
                temp.write("LV\\color",0,7)
                temp.write("LV\\size\\h",0,400)
                temp.write("LV\\back",0,74)
                temp.write("RV\\back",0,44)
                temp.write("LV\\back\\egege\\gegege",0,77)
                temp.write("TS\\back\\egege\\gegege",0,74)
                temp.write("TS\\back\\egege",0,74)
                temp.write("LV\\for",0,5)
                temp.write("LV\\size\\w",0,600)
                temp.write("LV\\color\\force",0,999)
                bm_write +=1
        if (type == 'A' or type == 'R' ):
            time_start = time.perf_counter()
            while(time.perf_counter() - time_start < interval):
